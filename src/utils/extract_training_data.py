...
from utils.conll_reader import DependencyStructure, conll_reader
...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        buf, stack = state.buffer, state.stack
        output_arr = np.array([])
        
        buf_length = len(buf)
        buf_idxs = min(3, buf_length)
        buf_wrds = buf[-1*buf_idxs:]
        for idx in range(len(buf_wrds)-1, -1, -1):
            if words[buf_wrds[idx]] == None:
                word = None
            else:
                word = words[buf_wrds[idx]].lower()
            if buf_wrds[idx] == 0:
                output_arr=np.append(output_arr, int(self.word_vocab["<ROOT>"]))
            elif pos[buf_wrds[idx]] == "NNP":
                output_arr=np.append(output_arr, int(self.word_vocab["<NNP>"]))
            elif pos[buf_wrds[idx]] == "CD":
                output_arr=np.append(output_arr, int(self.word_vocab["<CD>"]))
            elif word not in self.word_vocab:
                output_arr=np.append(output_arr, int(self.word_vocab["<UNK>"]))
            else:                  
                output_arr=np.append(output_arr, int(self.word_vocab[word]))
        
        for i in range(3-buf_idxs):
            output_arr=np.append(output_arr, int(self.word_vocab["<NULL>"]))
            
        
        stack_length = len(stack)
        stack_idxs = min(3, stack_length)
        stack_wrds = stack[-1*stack_idxs:]
        for idx in range(len(stack_wrds)-1, -1, -1):
            if words[stack_wrds[idx]] == None:
                word = None
            else:
                word = words[stack_wrds[idx]].lower()
            if stack_wrds[idx] == 0:
                output_arr=np.append(output_arr, int(self.word_vocab["<ROOT>"]))
            elif pos[stack_wrds[idx]] == "NNP":
                    output_arr=np.append(output_arr, int(self.word_vocab["<NNP>"]))
            elif pos[stack_wrds[idx]] == "CD":
                    output_arr=np.append(output_arr, int(self.word_vocab["<CD>"]))
            elif word not in self.word_vocab:
                output_arr=np.append(output_arr, int(self.word_vocab["<UNK>"]))
            else:                  
                output_arr=np.append(output_arr, int(self.word_vocab[word]))
        for i in range(3-stack_idxs):
            output_arr=np.append(output_arr, int(self.word_vocab["<NULL>"]))
        return output_arr

    def get_output_representation(self, output_pair):  
        # TODO: Write this method for Part 2
        a = keras.utils.np_utils.to_categorical(self.output_labels[output_pair], num_classes=91)
        return a
...
def get_training_matrices(extractor, in_file):
    ...
    for dtree in conll_reader(in_file):
       # print(dtree.print_conll() )
        ...
        count += 1
    sys.stdout.write("\n")
    #print(all_words[100],states[100],np.flip(inputs[100]).tolist())
